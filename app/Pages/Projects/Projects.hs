...
buildSidebarSection (x : xs) indent path =
  [hsx|
...
buildSidebarItem (Tree (name, _) children) indent path =
  [hsx|
...
findItem_ (target : xs) (Tree value children) = do
  ...
  if name == target
    then case find (\(x, _) -> x /= fst defaultProject) $ map (findItem xs) children of

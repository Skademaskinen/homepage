...
findInChildren f ((Tree value []) : xs) | f value   = Just value
                                        | otherwise = findInChildren f xs
findInChildren f ((Tree value children) : xs)   | f value   = Just value
                                                | otherwise = findInChildren f children
findInChildren _ [] = Nothing

findInTree :: (a -> Bool) -> Tree a -> Maybe a
findInTree f (Tree value children)  | f value   = Just value
                                    | otherwise = findInChildren f children
